"""Нормировка входов по типу клетки (перенос модели Shiu между сканами): toy-графы."""
import numpy as np
import pytest
...
def _pair():
    # опорный граф: три клетки типа A с входами 10, 20, 30 → медиана 20; src — источник без входов
    ref = _toy(["src", "a1", "a2", "a3"], [("src", "a1", 10), ("src", "a2", -20), ("src", "a3", 30)], dataset="ref")
    ref_types = ["", "A", "A", "A"]
    # ds: клетка типа A с входом 5 (→ ×4), типа A с входом 200 (→ клип 0.2), без типа (→ 1.0),
    # типа B, которого нет в ref (→ 1.0); щелевой контакт lo–hi
...
def test_normalize_factors_and_clip():
    ...
    assert row("lo")[g.index["src"]] == pytest.approx(20.0)            # 5 × 4.0
    assert row("hi")[g.index["src"]] == pytest.approx(-200 * 0.2)      # 20/200=0.1 → клип 0.2
    assert row("notype")[g.index["src"]] == pytest.approx(7.0)         # без типа — 1.0
    assert row("b")[g.index["src"]] == pytest.approx(3.0)              # типа нет в ref — 1.0
    assert (g.W_gap != Wg_before).nnz == 0                             # W_gap не тронута
    assert (ds.W_chem[ds.index["lo"]].toarray().ravel()[ds.index["src"]]) == 5.0   # исходный граф не изменён
    ...
    assert p["factor_median"] == pytest.approx(1.0)                    # медиана по всем клеткам: [1,1,1,0.2,4] → 1
    assert "rule" in p and g.params["min_count"] == 5                  # старые params сохранены


def test_normalize_cache_roundtrip(tmp_path, monkeypatch):
    monkeypatch.setattr(paths, "CACHE", tmp_path / "cache")
    monkeypatch.setattr(paths, "DB_PATH", tmp_path / "nope.sqlite")     # базы нет: get идёт только через кэш
    ds, ds_types, ref, ref_types = _pair()
    ...
    g2 = graph.get("ds", normalize="ref")                               # второй раз — из кэша
    assert g2.version == "v9 norm:ref" and g2.params["normalize"]["cells_scaled"] == 2
    assert (g2.W_chem != g.W_chem).nnz == 0
    assert graph.get("ds").version == "v9"                             # обычный кэш не затронут
