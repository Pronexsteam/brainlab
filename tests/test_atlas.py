...
def test_atlas_table_from_fake_runner(tmp_path, monkeypatch):
    ...
    assert "min_count=5" in md    # оговорка про порог/правило под таблицей
    assert json.loads((tmp_path / "results" / "atlas" / "taste.json").read_text(encoding="utf-8"))["d2"]["sugar"] == 0.0
    atlas.run("taste", ["d1"], save=False)
    assert (tmp_path / "results" / "atlas" / "taste-1.md").exists()     # старую таблицу не перезаписали


def test_atlas_rejects_dataset_filename_mismatch(tmp_path, monkeypatch):
    """yaml dataset: должен совпадать с токеном файла <name>_<токен>.yaml — иначе measure тихо
    читал бы группы не того набора (dataset из spec против ds из имени файла)."""
    monkeypatch.setattr(paths, "RESULTS", tmp_path / "results"); monkeypatch.setattr(paths, "LAB", tmp_path / "lab")
    (tmp_path / "lab" / "experiments").mkdir(parents=True)
...
def test_atlas_accepts_token_starting_with_dataset(tmp_path, monkeypatch):
    """Токен banc_888_norm → файл <name>_banc_888_norm.yaml с dataset: banc_888 — допускается:
    проверка ослаблена до «токен начинается с dataset» (нормированный вариант того же набора)."""
    monkeypatch.setattr(paths, "RESULTS", tmp_path / "results"); monkeypatch.setattr(paths, "LAB", tmp_path / "lab")
    (tmp_path / "lab" / "experiments").mkdir(parents=True)
