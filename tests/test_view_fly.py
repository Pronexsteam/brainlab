...
def _get(srv, path):
    ...
    path = urllib.parse.quote(path, safe="/=,?&")  # путь может содержать кириллицу (имя группы/набора)
    try:
...
def test_populations_and_big_graph_requires_focus(srv):
    ...
    st, body = _get(srv, "/api/populations/нет")
    assert st == 404
    if not (paths.CACHE / "fafb_783.npz").exists():
        pytest.skip("нет fafb_783")
...
def test_big_run_without_names_requires_names(srv):
    ...
    if big_run is None:
        pytest.skip("нет прогонов fafb_783")
...
def test_page_fly_subgraph_and_state(srv):
    ...
    with playwright.sync_playwright() as p:
        ...
        if "fafb_783" not in dataset_ids:
            browser.close()
            pytest.skip("fafb_783 не в списке наборов страницы")

        page.select_option("#dataset", "fafb_783")
        page.wait_for_function(
            "document.getElementById('status').textContent.includes('подграф')", timeout=30000)
        ...
        assert "fafb_783" in status and "подграф" in status
        ...
        if fly_runs:
            ...
            page.wait_for_function(
                "document.getElementById('status').textContent.includes('прогон')", timeout=15000)
